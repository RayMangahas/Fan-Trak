...
const CREATOR_TYPES = ["Hobby Creator", "Part-Time Creator", "Full-Time Creator", "Agency / Manager", "Other"];
...
function WaitlistForm() {
  ...
  const [form, setForm] = useState({ name: "", email: "", creatorType: "", creatorTypeOther: "", primaryPlatform: "", stageName: "", hasManager: "", managerName: "", companyName: "", companyEmail: "", additionalInfo: "" });
  ...
  const mutation = useMutation({
    mutationFn: (data: typeof form) =>
      apiRequest("POST", "/api/waitlist", {
        name: data.name,
        email: data.email,
        creatorType: data.creatorType === "Other" ? (data.creatorTypeOther || "Other") : (data.creatorType || null),
        primaryPlatform: data.primaryPlatform || null,
        stageName: data.stageName || null,
        hasManager: data.hasManager || null,
  ...
  return (
    ...
    >
      ...
      <p className="text-xs font-semibold mb-1" style={{ color: "#00BFFF", letterSpacing: "0.08em", textTransform: "uppercase" }}>Your Information</p>
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <input
          className="ft-input"
          placeholder="Stage / Creator Name (optional)"
      ...
      <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
        ...
        <select
          ...
          onChange={(e) => setForm(f => ({ ...f, hasManager: e.target.value }))}
          data-testid="select-has-manager"
          style={{ appearance: "none" }}
        >
          <option value="">Do you have a Manager? *</option>
      ...
      {form.creatorType === "Other" && (
        <input
          className="ft-input"
          placeholder="Please describe your creator type"
          value={form.creatorTypeOther}
          onChange={(e) => setForm(f => ({ ...f, creatorTypeOther: e.target.value }))}
        />
      )}

      {form.hasManager === "Yes" && (
        <>
          <p className="text-xs font-semibold mt-1" style={{ color: "hsl(220 15% 50%)", letterSpacing: "0.06em", textTransform: "uppercase" }}>Manager / Company Info</p>
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
            <input
              className="ft-input"
      ...
      <div>
        <p className="text-xs font-semibold mb-1" style={{ color: "hsl(220 15% 40%)", letterSpacing: "0.06em", textTransform: "uppercase" }}>Anything else you'd like us to know? (optional)</p>
        <textarea
          className="ft-textarea"
          rows={4}
          maxLength={700}
          placeholder="Tell us about your setup, goals, or anything that would help us build a better product for you..."
          value={form.additionalInfo}
          onChange={(e) => setForm(f => ({ ...f, additionalInfo: e.target.value }))}
          data-testid="textarea-additional-info"
        />
        <p className="text-xs text-right" style={{ color: "hsl(220 15% 55%)", marginTop: "-6px" }}>{form.additionalInfo.length}/700</p>
      </div>
...
function FloorResearch() {
  const { toast } = useToast();
  const [step, setStep] = useState(-1); // -1 = personal info screen
  const [personalInfo, setPersonalInfo] = useState({ fullName: "", stageName: "", researchEmail: "", creatorType: "", hasManager: "", managerName: "", companyName: "", companyEmail: "" });
  ...
  const mutation = useMutation({
    mutationFn: (data: ResearchAnswers) => {
      ...
      // Append "Other" detail text where applicable
      const biggestPain = data.biggestPain === "Other" && data.biggestPain_other
        ? `Other: ${data.biggestPain_other}`
        : (data.biggestPain as string) || null;
      const openResponse = (data.openResponse as string) || null;
      return apiRequest("POST", "/api/research", {
        // Personal info
        fullName: personalInfo.fullName || null,
        stageName: personalInfo.stageName || null,
        researchEmail: personalInfo.researchEmail || null,
        creatorType: personalInfo.creatorType || null,
        hasManager: personalInfo.hasManager || null,
        managerName: personalInfo.managerName || null,
        companyName: personalInfo.companyName || null,
        companyEmail: personalInfo.companyEmail || null,
        // Survey answers
  ...
  const q = step >= 0 ? RESEARCH_QUESTIONS[step] : null;
  const pct = step < 0 ? 0 : ((step + 1) / totalSteps) * 100;
  ...
  function restart() {
    setAnswers({});
    setPersonalInfo({ fullName: "", stageName: "", researchEmail: "", creatorType: "", hasManager: "", managerName: "", companyName: "", companyEmail: "" });
    setStep(-1);
    setSubmitted(false);
  }
  ...
  if (step === -1) {
    const canContinue = personalInfo.fullName.trim() && personalInfo.researchEmail.includes("@");
    return (
      <div className="flex flex-col gap-3 fade-in">
        <p className="text-xs font-semibold" style={{ color: "#00BFFF", letterSpacing: "0.08em", textTransform: "uppercase" }}>Your Information</p>
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <input className="ft-input" placeholder="Full Name *" value={personalInfo.fullName} onChange={e => setPersonalInfo(p => ({ ...p, fullName: e.target.value }))} />
          <input className="ft-input" placeholder="Stage / Creator Name (optional)" value={personalInfo.stageName} onChange={e => setPersonalInfo(p => ({ ...p, stageName: e.target.value }))} />
        </div>
        <input className="ft-input" type="email" placeholder="Email Address *" value={personalInfo.researchEmail} onChange={e => setPersonalInfo(p => ({ ...p, researchEmail: e.target.value }))} />
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          <select className="ft-input" value={personalInfo.creatorType} onChange={e => setPersonalInfo(p => ({ ...p, creatorType: e.target.value }))} style={{ appearance: "none" }}>
            <option value="">Creator Type (optional)</option>
            {CREATOR_TYPES.map(t => <option key={t} value={t}>{t}</option>)}
          </select>
          <select className="ft-input" value={personalInfo.hasManager} onChange={e => setPersonalInfo(p => ({ ...p, hasManager: e.target.value }))} style={{ appearance: "none" }}>
            <option value="">Manager? (optional)</option>
            <option value="Yes">Yes</option>
            <option value="No">No</option>
          </select>
        </div>
        {personalInfo.hasManager === "Yes" && (
          <>
            <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
              <input className="ft-input" placeholder="Manager Name" value={personalInfo.managerName} onChange={e => setPersonalInfo(p => ({ ...p, managerName: e.target.value }))} />
              <input className="ft-input" placeholder="Company Name" value={personalInfo.companyName} onChange={e => setPersonalInfo(p => ({ ...p, companyName: e.target.value }))} />
            </div>
            <input className="ft-input" type="email" placeholder="Company / Manager Email" value={personalInfo.companyEmail} onChange={e => setPersonalInfo(p => ({ ...p, companyEmail: e.target.value }))} />
          </>
        )}
        <button className="btn-primary w-full mt-1" disabled={!canContinue} onClick={() => setStep(0)} type="button">
          Continue to Survey
...
export default function App() {
  ...
  return (
    <div className="min-h-screen">
      ...
      {/* ── NAV ── */}
      <nav className="sticky top-0 z-50 flex items-center justify-between px-6 py-4" style={{ background: "rgba(255,255,255,0.95)", backdropFilter: "blur(12px)", borderBottom: "1px solid hsl(220 15% 85%)" }}>
