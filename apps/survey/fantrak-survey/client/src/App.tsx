...
function ProgressBar({ step, total }: { step: number; total: number }) {
  ...
  return (
    <div className="mb-6">
      ...
      <div className="h-1.5 rounded-full" style={{ background: "hsl(232 25% 18%)" }}>
...
function StatsPanel() {
  ...
  return (
    <div className="rounded-lg p-4 mb-6 border" style={{ background: "hsl(232 25% 12%)", borderColor: "hsl(232 25% 20%)" }}>
        ...
        <div>
          ...
          {topRated.map(({ label, val }) => (
            <div key={label} className="flex items-center justify-between text-xs mb-1">
              <span style={{ color: "hsl(220 20% 80%)" }}>{label}</span>
              <div className="flex items-center gap-1">
                <Star size={11} className="fill-yellow-400 text-yellow-400" />
                <span style={{ color: "hsl(220 20% 80%)" }}>{val}</span>
              </div>
            </div>
          ))}
        </div>
...
function ThankYou({ onReset }: { onReset: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center px-4">
      <div className="mb-6 rounded-full p-4" style={{ background: "hsl(232 25% 14%)" }}>
      ...
      <div className="rounded-lg p-5 mb-8 w-full max-w-sm border text-left" style={{ background: "hsl(232 25% 12%)", borderColor: "hsl(326 100% 50% / 30%)" }}>
        <div className="flex items-center gap-2 mb-3">
...
export default function App() {
  ...
  const canAdvance = () => {
    if (step === 1) return !!form.creatorType && form.currentPlatforms.length > 0;
    if (step === 2) return FEATURES.every(f => !!form.ratings[f.key]);
    if (step === 3) return !!form.biggestPainPoint && !!form.mostWantedFeature && form.overallScore > 0;
    return true;
  };
    ...
    return (
      <div className="min-h-screen flex items-start justify-center p-4 pt-12">
        <div className="w-full max-w-xl">
          <Header />
          <div className="rounded-xl border" style={{ background: "hsl(232 30% 10%)", borderColor: "hsl(232 25% 18%)" }}>
            <ThankYou onReset={() => { setSubmitted(false); setStep(1); setForm({ creatorType:"", currentPlatforms:[], ratings:{}, comments:{}, overallScore:0, biggestPainPoint:"", mostWantedFeature:"", additionalComments:"", wouldJoinBeta:null, email:"" }); }} />
          </div>
        </div>
      </div>
    );
  ...
  return (
    <div className="min-h-screen flex items-start justify-center p-4 pt-12 pb-16">
      <div className="w-full max-w-xl">
        <Header />
        <StatsPanel />
        <div className="rounded-xl border p-6" style={{ background: "hsl(232 30% 10%)", borderColor: "hsl(232 25% 18%)" }}>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {["Hobby Creator", "Part-Time Creator", "Full-Time Creator", "Agency / Manager"].map(ct => (
                    <button
                      ...
                      style={{
                        background: form.creatorType === ct ? "hsl(326 100% 50% / 15%)" : "hsl(232 25% 14%)",
                        borderColor: form.creatorType === ct ? "hsl(326 100% 50%)" : "hsl(232 25% 22%)",
                        color: form.creatorType === ct ? "hsl(326 100% 65%)" : "hsl(220 15% 70%)",
                      }}
                    >
                      {ct}
                    </button>
                ...
                <div className="flex flex-wrap gap-2">
                  {PLATFORMS.map(p => (
                    <button
                      ...
                      style={{
                        background: form.currentPlatforms.includes(p) ? "hsl(197 100% 50% / 15%)" : "hsl(232 25% 14%)",
                        borderColor: form.currentPlatforms.includes(p) ? "hsl(197 100% 50%)" : "hsl(232 25% 22%)",
                ...
                {FEATURES.map((f, i) => (
                  <div key={f.key}>
                    ...
                    <textarea
                      ...
                      style={{
                        background: "hsl(232 25% 14%)",
                        borderColor: "hsl(232 25% 22%)",
                        color: "hsl(220 20% 88%)",
                ...
                <div className="space-y-2">
                  {PAIN_POINTS.map(p => (
                    <button
                      ...
                      style={{
                        background: form.biggestPainPoint === p ? "hsl(326 100% 50% / 12%)" : "hsl(232 25% 14%)",
                        borderColor: form.biggestPainPoint === p ? "hsl(326 100% 50%)" : "hsl(232 25% 22%)",
                        color: form.biggestPainPoint === p ? "hsl(326 100% 70%)" : "hsl(220 15% 70%)",
                      }}
                    >
                      {p}
                    </button>
                ...
                <div className="space-y-2">
                  {WANTED_FEATURES.map(feat => (
                    <button
                      ...
                      className="w-full text-left rounded-md px-3 py-2.5 text-sm border transition-all"
                      style={{
                        background: form.mostWantedFeature === feat ? "hsl(197 100% 50% / 12%)" : "hsl(232 25% 14%)",
                        borderColor: form.mostWantedFeature === feat ? "hsl(197 100% 50%)" : "hsl(232 25% 22%)",
                        color: form.mostWantedFeature === feat ? "hsl(197 100% 70%)" : "hsl(220 15% 70%)",
                      }}
                    >
                      {feat}
                    </button>
          ...
          {step === 4 && (
            <div>
                    ...
                    <button
                      ...
                      style={{
                        background: form.wouldJoinBeta === val
                          ? val ? "hsl(326 100% 50% / 15%)" : "hsl(232 25% 16%)"
                          : "hsl(232 25% 14%)",
                        borderColor: form.wouldJoinBeta === val
                          ? val ? "hsl(326 100% 50%)" : "hsl(232 25% 35%)"
                          : "hsl(232 25% 22%)",
                        color: form.wouldJoinBeta === val
                          ? val ? "hsl(326 100% 65%)" : "hsl(220 15% 75%)"
                          : "hsl(220 15% 55%)",
                      }}
                    >
              ...
              {form.wouldJoinBeta && (
                <div className="mb-5">
                  ...
                  <input
                    ...
                    className="w-full rounded-md border px-3 py-2.5 text-sm focus:outline-none focus:ring-1"
                    style={{
                      background: "hsl(232 25% 14%)",
                      borderColor: "hsl(232 25% 22%)",
                      color: "hsl(220 20% 88%)",
              ...
              <div className="rounded-lg p-4 border" style={{ background: "hsl(232 25% 12%)", borderColor: "hsl(232 25% 20%)" }}>
          ...
          {/* Navigation */}
          <div className="flex items-center justify-between mt-7 pt-5 border-t" style={{ borderColor: "hsl(232 25% 18%)" }}>
            {step > 1 ? (
              <button
                type="button"
                ...
                style={{ borderColor: "hsl(232 25% 25%)", color: "hsl(220 15% 65%)", background: "transparent" }}
              >
            ...
            {step < TOTAL_STEPS ? (
              <button
                ...
                style={{
                  background: canAdvance() ? "hsl(326 100% 50%)" : "hsl(232 25% 20%)",
